...
Open RTSP mic (Seeed XIAO ESP32-S3 + XMOS XVF3800) used by **UAV-radar / NEVOD** acoustic nodes and as a standalone nature mic. Init: header of [`src/main.cpp`](../src/main.cpp). LEDs: [LED.md](LED.md). HTTP: [`openapi-webui.yaml`](../openapi-webui.yaml). MQTT/NVS: [API_REFERENCE.md](API_REFERENCE.md).

Context: [README.md](../README.md) · [BirdLense Hub](https://github.com/Gfermoto/BirdLense-Hub) · [BirdNET](https://github.com/kahst/BirdNET-Analyzer)
...
3. HPF: live XVF → chip only; soft filter only if the chip is not initialized  
4. `OpusEncoder` (Core 0) → Opus 20 ms / 32 kbps  
5. `EncodedAudioFanout` → local RTSP and optional remote client (drop-oldest)  
6. RTP timestamps via `RtspRuntimeCore`; listen `:554`
...
| **ESP32-S3** | захват, MEL, Opus, RTSP, WebUI, MQTT, опц. W5500 |
| **GPIO21** | статусный LED XIAO (не RGB-кольцо) |
| **RGB ring** | 12× WS2812 на Seeed; `LED_EFFECT` по I2C |
...
Пайплайн, диаграмма и таблица задач — те же, что в английской секции выше.

### Lifecycle

`AudioLifecycle` — согласованный pause/stop при нагреве и OTA.  
`TcpBudget` — лимит одновременных RTSP TCP (защита lwIP).

### Сеть и UI

Captive Wi‑Fi, опциональный Ethernet, WebUI Basic+CSRF+WS ticket, общие пароли Web/RTSP, MQTT `rtsp-mic/v1/{node_id}/…`.

### Конфиг

NVS **`rtspmic`**. Константы: `Config.h` + `ConfigDev.h`. По возможности `softApply` без полного reboot.
...
4. HTTP ↔ OpenAPI.  
5. Смена schema NVS → миграция / factory reset.
