...
Готовый бинарник: канал OTA / USB **[`diy-ota`](https://github.com/Gfermoto/UAV-radar/releases/tag/diy-ota)** (сейчас **0.17.84** · versioned [`v0.17.84-ota`](https://github.com/Gfermoto/UAV-radar/releases/tag/v0.17.84-ota)).
...
| 3–4 | Wi‑Fi / портал (кольцо, RTSP), WebUI, калибровка |
| 5 | OTA |
...
MAC на наклейке / в роутере выглядит как `1C:DB:D4:74:6E:8C`. Из него плата делает **три разные** строки:

| Что | Как выглядит | Зачем |
|-----|----------------|-------|
| **SSID и пароль точки `NV…`** | `NV` + **8** hex из MAC, **ЗАГЛАВНЫЕ**, например `NVD4746E8C` | Wi‑Fi точка и RTSP в портале |
| **Адрес WebUI после домашней сети** | те же символы, но **строчными**: `http://nvd4746e8c.local/` | браузер в LAN |
| **Короткий id в MQTT / статусе** | **6** hex, например `746E8C` | телеметрия. **Это не пароль точки** |

`NV746E8C` в списке сетей не будет. Логин и пароль точки — **полная** строка `NVD4746E8C` (как на экране телефона).

#### 3.2. Когда плата поднимает точку

| Ситуация | Что делает |
|----------|------------|
| Первый старт / после полной прошивки / factory reset (сети ещё нет) | Сразу точка `NV…` |
| Сеть уже сохранена, но эфир молчит (роутер выкл, неверный пароль, только 5 ГГц) | ~45 с пробует домашнюю сеть. До **трёх** таких бутов, затем снова точка (recovery). После таймаута точки снова пробует домашнюю |
| Сохранили Wi‑Fi на портале | Перезагрузка → на следующем старте идёт в домашнюю сеть |

Пароль точки = SSID. Кто видит сеть `NV…`, знает пароль и может открыть портал и микрофон. Так задумано для первой настройки.

Не видите `NV…`? Подождите 30–60 с после RESET. Для дома нужна раздача **2.4 ГГц** (плата не видит 5 ГГц).

#### 3.3. Что делает плата на своей точке

Подключитесь к `NV…` (пароль = имя сети). Если страница сама не открылась — **`http://192.168.4.1`**.

| | Поведение |
|--|--|
| **Портал** | Скан 2.4 ГГц, выбор домашней сети, сохранить. Плата перезапустится и уйдёт с `NV…`. Телефон верните в домашний Wi‑Fi |
| **Кольцо** | Если в настройках «Кольцо» = Вкл: **красное дыхание** (не азимут). Выкл → темно. Подробнее: [LED.md](LED.md) |
| **Микрофон** | Поток сразу, без отдельного «режима настройки». В VLC / ffplay обычный RTSP, крутить транспорт не нужно |
| **Пароль потока** | **SSID и пароль точки**, не `admin` / `nevod-change-me` |
| **Сколько слушателей** | Один. На точку тоже садится один телефон/ноут |
| **Сколько живёт точка** | **180 с без** клиента → перезагрузка и снова точка (или повтор домашней сети). Пока вы сидите на `NV…` (даже просто слушаете mic) — плата **не** бросает. Отключились → снова 180 с. «Сохранить» домашнюю сеть — всё равно перезагрузка |

```text
rtsp://NVD4746E8C:NVD4746E8C@192.168.4.1:554/
```

Подставьте **своё** имя сети вместо примера. После выхода в домашнюю сеть этот адрес больше не работает: поток тогда — `admin` и **ваш** пароль WebUI, см. ниже.

На телефоне:

1. Подключитесь к `NV…`.
2. (По желанию) откройте поток в VLC — так проверяете слух ещё до домашней сети.
3. На портале выберите **домашнюю сеть 2.4 ГГц**, введите пароль, сохраните.
4. Дождитесь перезапуска. Телефон снова в домашний Wi‑Fi.
...
Hostname — **тот же идентификатор, что SSID, но строчными**: `NVD4746E8C` → `http://nvd4746e8c.local/`.
...
#### 3.5. Быстрая проверка «жив ли слух»

Ещё на точке `NV…` можно открыть RTSP (шаг 3.3). После домашней сети:
...
3. Кольцо на XVF в домашней сети следует звуку (азимут), не красному дыханию портала. Выкл в WebUI — темно.

Тишина после хлопков → вернитесь к шагу 1 (образ I²S slave 16 kHz) и проверьте кабель XIAO.

Локальный поток в домашней сети (после смены пароля WebUI; заводской `nevod-change-me` поток не отдаст):

```text
rtsp://admin:ВАШ_ПАРОЛЬ@nvd4746e8c.local:554/
```

Это **не** пароль точки `NV…`. Режим потока на столе — чтобы подкрутить звук, не для круглосуточного поля. После портала плата сама возвращается к детекции.
...
3. **Система → GPS / координаты**: задайте **широту, долготу и высоту** места установки. Сохраните.
   *Пример для проверки формы (не реальная точка): посёлок **Атлантида**, `55.867276, 36.485023`, высота `120` м — вымышленный двор-сектор в гайде.*
...
- [ ] Зашли в AP `NV…` с паролем **как SSID, ЗАГЛАВНЫМИ** (не короткий id из 6 букв)
- [ ] На точке: красное дыхание кольца (если Вкл) и/или поток `rtsp://SSID:SSID@192.168.4.1:554/`
- [ ] Домашний Wi‑Fi 2.4 ГГц, WebUI открывается 
- [ ] Пароль `admin` сменён с `nevod-change-me` 
- [ ] Хлопок двигает азимут / уровень; кольцо снова азимут, не красное дыхание
...
| Нет сети `NV…` | Подождать после RESET; питание на XIAO. Если домашняя сеть уже сохранена — плата сначала ~45 с ищет её |
| AP есть, пароль не подходит | Полная строка как SSID, **заглавные**. Не `admin`, не короткий id `746E8C` |
| Портал не открывается | Вручную `http://192.168.4.1`. Пока сидите на `NV…`, точка не гаснет; без клиента ~3 мин → перезагрузка |
| VLC «подключился», тишина | Логин/пароль = **SSID**, адрес `192.168.4.1`. Не `admin`. Один слушатель |
| Кольцо красное «дышит» на точке | Так и должно: это портал, не азимут. В домашней сети снова следует звуку |
...
Signed binary: OTA / USB channel **[`diy-ota`](https://github.com/Gfermoto/UAV-radar/releases/tag/diy-ota)** (currently **0.17.84** · versioned [`v0.17.84-ota`](https://github.com/Gfermoto/UAV-radar/releases/tag/v0.17.84-ota)).
...
Do not mix these names (example MAC `1C:DB:D4:74:6E:8C`):

| What | Example | Use |
|------|---------|-----|
| AP SSID **and** AP password | `NVD4746E8C` (UPPERCASE, `NV` + 8 hex) | Join the board’s AP; RTSP while on the AP |
| WebUI hostname after home Wi‑Fi | `http://nvd4746e8c.local/` | Browser on LAN |
| Short MQTT / status id | `746E8C` | Telemetry only — **not** the AP password |

| When | What the board does |
|------|---------------------|
| First boot / full flash / factory reset | AP `NV…` immediately |
| Saved Wi‑Fi but the air is silent | ~45 s STA, up to 3 boots, then recovery AP |
| You save home Wi‑Fi on the portal | Reboot, then STA |

On the AP (`http://192.168.4.1`):

- Ring On → **red breath** (not DoA). Ring Off → dark. See [LED.md](LED.md).
- Mic: `rtsp://NVD4746E8C:NVD4746E8C@192.168.4.1:554/` (use **your** SSID). Basic = AP, not `admin`.
- One listener. Stay associated and the 180 s idle timer does not fire. `/save` still reboots.
- After home Wi‑Fi the board is back in detection (RTSP on the AP does not stick).

Join `NV…` → optional VLC check → pick home **2.4 GHz** → save.  
WebUI: `http://nv….local/` (lowercase) or LAN IP.  
Login **`admin` / `nevod-change-me`** — **change immediately** (System).  
On LAN, clap: level / DoA must move. Home RTSP (after you change the WebUI password): `rtsp://admin:YOUR_PASSWORD@nv….local:554/`.
...
3. **System → GPS**: set **latitude, longitude, and altitude**; save.
   *Form-check example only (not a real site): settlement **Atlantida**, `55.867276, 36.485023`, alt `120` m — a fictional yard-sector in this guide.*
